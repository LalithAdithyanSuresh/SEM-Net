...
def process_chunk_gpu(gpu_id, image_paths, output_dir, model_name, batch_size, status_queue, overwrite=True, use_sam2=False, sam2_checkpoint=None):
    ...
    try:
        ...
        if use_sam2:
            # Load SAM2 model
            sam2_model = load_sam2_model(sam2_checkpoint, device)
        else:
        ...
        if use_sam2:
            # Process with SAM2 model (single-image inference)
            for img_path in unprocessed:
                out_path = get_out_path(img_path, output_dir)
                try:
                    masks = run_sam2_inference(sam2_model, img_path, device)
                    if masks:
                        h, w = masks[0].shape
                        ...
                        for idx, m_bin in enumerate(masks):
                            unique_id = int((idx + 1) * 255 / max(len(masks), 1))
                            combined_mask[m_bin, 0] = unique_id
                            combined_mask[m_bin, 1:] = colors[idx, 1:]
                    ...
                    print(f"[{device}] Error processing {img_path} with SAM2: {e}", file=sys.stderr)
                status_queue.put(1)
            return
...
def generate_masks_for_dir(input_dir, output_dir, gpus=None, batch_size=16, model_name="sam_b.pt", overwrite=True, workers_per_gpu=1, num_workers=None, sam2_checkpoint=None):
    ...
    if gpus is None:
        ...
        gpu_list = gpus
    use_sam2 = sam2_checkpoint is not None
    ...
    for gpu_id, chunk in zip(worker_gpu_list, chunks):
        ...
        p = ctx.Process(
            ...
            args=(gpu_id, chunk, output_dir, model_name, batch_size, status_queue, overwrite, use_sam2, sam2_checkpoint)
        )
        p.start()
        processes.append(p)
...
def main():
    ...
    parser.add_argument("--sam2-checkpoint", default=None, help="Path to SAM2 checkpoint YAML config file (e.g., sam2_hiera_l.yaml). If provided, SAM2 will be used instead of SAM/FastSAM.")
