...
def get_custom_mask(indexed_masks, cat, index, h, w):
    ...
    # Sequential selection: map the i-th test image directly to the i-th mask in the category
...
def calc_psnr_ssim(gt, pre):
    ...
    # Note: multichannel=True is equivalent to channel_axis=-1 in newer skimage
    ssim = compare_ssim(gt, pre, channel_axis=-1, data_range=255)

    return psnr, ssim

# Removed calc_psnr_ssim_masked as per user request for whole-image focus
...
def main():
    ...
    for cat in categories:
        ...
        for index, items in enumerate(test_loader):
            ...
            # save_name uses Full Image PSNR as requested
